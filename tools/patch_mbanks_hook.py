...
# 2026-09-02: 접근자가 **네 곳**이다. 표 캐시 base(0x8015F314)를 읽는 자리를 세어 보면
# MAP 안에 7곳이고, 그중 뱅크 베이스를 마스크하는 접근자가 4쌍이다.
# 처음에는 0x801540A4 하나만 고쳤는데, 스크립트 VM 이 쓰는 나머지 세 곳이 남아
# 전함 발진에서 잘못된 주소를 만들어 게임이 에러 트랩(무한 루프)에 빠졌다.
# 네 곳 모두 명령 단위로 같은 모양이다:
#     lw a3,0(v0) / lui v0,0xFFFF / and a3,a3,v0 / lui v0,0x8016 ...
# `lui` 가 싣는 $v0 는 바로 다음 줄에서 덮어쓰이므로 두 줄 다 지워도 안전하다.
PATCH = [
    (0x801540A4, 0x3C02FFFF, 0x00000000, "nop (lui $v0,0xFFFF 제거) — 소비자"),
    (0x801540A8, 0x00C23024, 0x00000000, "nop (and $a2,$a2,$v0 제거) — 소비자"),
    (0x80152560, 0x3C02FFFF, 0x00000000, "nop (lui $v0,0xFFFF 제거) — VM 1"),
    (0x80152564, 0x00E23824, 0x00000000, "nop (and $a3,$a3,$v0 제거) — VM 1"),
    (0x8015276C, 0x3C02FFFF, 0x00000000, "nop (lui $v0,0xFFFF 제거) — VM 2"),
    (0x80152770, 0x00E23824, 0x00000000, "nop (and $a3,$a3,$v0 제거) — VM 2"),
    (0x80152E80, 0x3C02FFFF, 0x00000000, "nop (lui $v0,0xFFFF 제거) — VM 3"),
    (0x80152E84, 0x00E23824, 0x00000000, "nop (and $a3,$a3,$v0 제거) — VM 3"),
]
